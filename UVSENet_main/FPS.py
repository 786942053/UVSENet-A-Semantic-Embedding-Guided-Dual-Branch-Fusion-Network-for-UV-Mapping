import time
import torch
from geoseg.models.STTNet import STTNet
from geoseg.models.UANet import UANet_VGG
from geoseg.models.BANet import BANet

model_infos = {
    # vgg16_bn, resnet50, resnet18
    'backbone': 'vgg16_bn',
    'pretrained': False,
    'out_keys': ['block1','block2','block3','block4'],
    'in_channel': 3,
    'n_classes': 2,
    'top_k_s': 64,
    'top_k_c': 16,
    'encoder_pos': True,
    'decoder_pos': True,
    'model_pattern': ['X', 'A', 'S', 'C'],

    'BATCH_SIZE': 3,
    'IS_SHUFFLE': True,
    'NUM_WORKERS': 0,
    'DATASET': 'generate_dep_info/train_data.csv',
    'model_path': 'Checkpoints',
    'log_path': 'Results',
    # if you need the validation process.
    'IS_VAL': True,
    'VAL_BATCH_SIZE': 3,
    'VAL_DATASET': 'generate_dep_info/val_data.csv',
    # if you need the test process.
    'IS_TEST': True,
    'TEST_DATASET': 'generate_dep_info/test_data.csv',
    'IMG_SIZE': [512, 512],
    'PHASE': 'seg',

    # China Dataset
    'PRIOR_MEAN': [0.37618707362175813, 0.38140236772409364, 0.3907937124297842],
    'PRIOR_STD': [0.0332261068138814, 0.030094983534503284, 0.028014925652876384],
    # # # WHU Dataset
    # 'PRIOR_MEAN': [0.4352682576428411, 0.44523221318154493, 0.41307610541534784],
    # 'PRIOR_STD': [0.026973196780331585, 0.026424642808887323, 0.02791246590291434],

    # if you want to load state dict
    'load_checkpoint_path': '',
    # if you want to resume a checkpoint
    'resume_checkpoint_path': '',
}


# model = BANet(num_classes=2)
# model = UANet_VGG(channel=32, num_classes=2)
model = STTNet(**model_infos)
model.eval().cuda()  # 推理模式，使用GPU
input_tensor = torch.randn(1, 3, 1500, 1500).cuda()  # 假设输入大小


for _ in range(10):
    _ = model(input_tensor)


start = time.time()
num_iters = 100
with torch.no_grad():
    for _ in range(num_iters):
        _ = model(input_tensor)
torch.cuda.synchronize()
end = time.time()

# 计算FPS
total_time = end - start
fps = num_iters / total_time
print(f"Inference FPS: {fps:.2f}")

torch.cuda.reset_peak_memory_stats()
with torch.no_grad():
    _ = model(input_tensor)
torch.cuda.synchronize()
max_memory = torch.cuda.max_memory_allocated() / (1024 ** 2)
print(f"Max memory allocated: {max_memory:.2f} MB")